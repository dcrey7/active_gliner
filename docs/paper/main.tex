...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/framework.jpg}
    \caption{Overview of the Active GLiNER framework. Data is first evaluated in zero-shot mode, then training examples are ranked by uncertainty using active learning strategies (LC, MNLP, MSE). The top-ranked examples are used for LoRA fine-tuning (Fit) with ground truth, LLM, or mixed labels. The fine-tuned model produces predictions (Predict) which are evaluated on the test set. The feedback loop from Predict back to Ranking enables iterative refinement.}
    \label{fig:framework}
\end{figure*}
